...
class TicketAttachment(KayakoObject):
    ...
    def get_all(cls, api, ticketid):
        ...
        return [
            ...
            for ticket_attachment_tree in tree.findall("attachment")
        ]
    ...
    def get(cls, api, ticketid, attachmentid):
        try:
            response = api._request(
                "%s/%s/%s/" % (cls.controller, ticketid, attachmentid), "GET"
            )
    ...
    def delete(self):
        if self.ticketid is None or self.ticketid is UnsetParameter:
            raise KayakoRequestError(
                "Cannot delete a TicketAttachment without being attached to a ticket. The ID of the Ticket (ticketid) has not been specified."
            )
