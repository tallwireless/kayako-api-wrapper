...
class TicketCountItem(KayakoObject):
    ...
    def _from_node(cls, node):
        return cls(
            cls._parse_int(node.get("id")),
            cls._parse_date(node.get("lastactivity")),
            cls._parse_int(node.get("totalitems")),
            totalunresolveditems=cls._parse_int(
                node.get("totalunresolveditems"), required=False
            ),
        )

    def __str__(self):
        return "<%s (%s): totalitems=%s lastactivity=%s%s>" % (
            self.__class__.__name__,
            self.id,
            self.totalitems,
            self.lastactivity,
            " totalunresolveditems=%s" % self.totalunresolveditems
            if self.totalunresolveditems is not None
            else "",
        )
...
class TicketCountDepartment(KayakoObject):
    ...
    def _from_node(cls, node):
        ...
        params = dict(
            ...
            statuses=tuple(
                TicketCountTicketStatus._from_node(ticketstatus_node)
                for ticketstatus_node in ticketstatus_nodes
            ),
            types=tuple(
                TicketCountTicketType._from_node(tickettype_node)
                for tickettype_node in tickettype_nodes
            ),
            staff=tuple(
                TicketCountOwnerStaff._from_node(ownerstaff_node)
                for ownerstaff_node in ownerstaff_nodes
            ),
        )
    ...
    def __str__(self):
        return (
            "<TicketCountDepartment (%s): totalitems:%s, lastactivity:%s, totalunresolveditems:%s, statuses:%s, types:%s, staff:%s>"
            % (
                self.id,
                self.totalitems,
                self.lastactivity,
                self.totalunresolveditems,
                len(self.statuses),
                len(self.types),
                len(self.staff),
            )
...
class TicketCount(KayakoObject):
    ...
    def _parse_ticket_count(cls, tree):
        ...
        if parent is not None:
            ...
            departments = tuple(
                TicketCountDepartment._from_node(node) for node in nodes
            )
        ...
        if parent is not None:
            nodes = parent.findall("department")
            unassigned = tuple(
                TicketCountUnassignedDepartment._from_node(node) for node in nodes
            )
    ...
    def __str__(self):
        return "<TicketCount: departments:%s, statuses:%s, staff:%s, unassigned:%s>" % (
