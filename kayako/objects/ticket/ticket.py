...
class Ticket(KayakoObject):
    ...
    def _parse_ticket(cls, api, ticket_tree):
        ...
        ticket_note_tree = None
        workflows = [
            dict(id=workflow_node.get("id"), title=workflow_node.get("title"))
            for workflow_node in ticket_tree.findall("workflow")
        ]
        watchers = [
            dict(staffid=watcher_node.get("staffid"), name=watcher_node.get("name"))
            for watcher_node in ticket_tree.findall("watcher")
        ]
        notes = [
            TicketNote(api, **TicketNote._parse_ticket_note(ticket_note_tree, ticketid))
            for ticket_note_tree in ticket_tree.findall("note")
            if ticket_note_tree.get("type") == "ticket"
        ]
        timetracks = [
            TicketTimeTrack(
                api,
                **TicketTimeTrack._parse_ticket_time_track(
                    ticket_time_track_tree, ticketid
                )
            )
        ...
        if posts_node is not None:
            posts = [
                TicketPost(
                    api, **TicketPost._parse_ticket_post(ticket_post_tree, ticketid)
                )
                for ticket_post_tree in posts_node.findall("post")
            ]
        ...
        params = dict(
            ...
            ticketstatusid=cls._get_int(
                ticket_tree.find("ticketstatusid"), required=False
            ),
            ticketpriorityid=cls._get_int(
                ticket_tree.find("priorityid")
            ),  # Note the difference, request param is ticketpriorityid, response is priorityid
            ...
            userorganizationid=cls._get_int(
                ticket_tree.find("userorganizationid"), required=False
            ),
    ...
    def add(self):
        ...
        if "userid" not in parameters and "staffid" not in parameters:
            raise KayakoRequestError(
                "To add a Ticket, at least one of the following parameters must be set: userid, staffid. (id: %s)"
                % self.id
            )
    ...
    def __str__(self):
        return "<Ticket (%s): %s - %s>" % (
            self.id,
            "UNSUBMITTED" if not self.displayid else self.displayid,
            self.subject,
        )
