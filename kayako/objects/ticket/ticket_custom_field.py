...
class TicketCustomFieldGroup(KayakoObject):
    ...
    def __str__(self):
        return "<TicketCustomFieldGroup (%s): %s (%s field%s)>" % (
            self.id,
            self.title,
            len(self.fields) if self.fields else 0,
            "" if self.fields and len(self.fields) == 1 else "s",
        )
...
class TicketCustomField(KayakoObject):
    ...
    def _parse_ticket_custom_field(cls, ticket_custom_field_tree, ticket_id):
        ...
        params = dict(
            ...
            type=CustomFieldTypes.name_by_id(
                cls._parse_int(ticket_custom_field_tree.get("type"))
            ),
    ...
    def get_all(cls, api, ticketid):
        ...
        for group_tree in tree.findall("group"):
            fields = [
                TicketCustomField(
                    api, **cls._parse_ticket_custom_field(custom_field, ticketid)
                )
                for custom_field in group_tree.findall("field")
            ]
            ticket_group = TicketCustomFieldGroup(
                cls._parse_int(group_tree.get("id")), group_tree.get("title"), fields
            )
    ...
    def __str__(self):
        return "<TicketCustomField (%s): %s %s>" % (
            self.id,
            self.type,
            self.short_value,
        )
