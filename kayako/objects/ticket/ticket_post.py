...
class TicketPost(KayakoObject):
    ...
    def delete(self):
        if self.ticketid is None or self.ticketid is UnsetParameter:
            raise KayakoRequestError(
                "Cannot delete a TicketPost without being attached to a ticket. The ID of the Ticket (ticketid) has not been specified."
            )
        self._delete("%s/%s/%s/" % (self.controller, self.ticketid, self.id))

    def __str__(self):
        return "<TicketPost (%s)%s>" % (
            self.id,
            ""
            if not self.contents
            else ": %s"
            if len(self.contents) < 20
            else ": %s..." % self.contents[:20],
        )
