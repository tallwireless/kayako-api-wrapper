...
from kayako.exception import (
    KayakoMethodNotImplementedError,
    KayakoRequestError,
    KayakoResponseError,
)
...
class KayakoRequestParser(NodeParser):
    ...
    def _parse_date(data, required=True, strict=True):
        try:
            ...
            raise KayakoResponseError(
                "There was an error parsing the response (_parse_date(%s, required=%s, strict=%s):\n\t%s"
                % (data, required, strict, error)
            )
...
class KayakoObject(ParameterObject, KayakoRequestParser):
    ...
    def get_all(cls, api, *args, **kwargs):
        ...
        raise KayakoMethodNotImplementedError(
            "GET ALL %s is not implemented for this object." % cls.__name__
        )
    ...
    def get(cls, api, *args):
        ...
        raise KayakoMethodNotImplementedError(
            "GET %s not implemented (id:%s)" % (cls.__name__, id)
        )
    ...
    def add(self):
        ...
        raise KayakoMethodNotImplementedError(
            "POST %s not implemented" % (self.__class__.__name__)
        )
    ...
    def _save(self, controller, *required_parameters):
        ...
        if self.id is None or self.id is UnsetParameter:
            raise KayakoRequestError(
                "Cannot save a non-existent %s. Use add instead."
        ...
        for required_parameter in self.__required_save_parameters__:
            if required_parameter not in parameters:
                raise KayakoRequestError(
                    "Cannot save %s: Missing required field: %s. (id: %s)"
                    % (self.__class__.__name__, required_parameter, self.id)
                )
    ...
    def save(self):
        ...
        raise KayakoMethodNotImplementedError(
            "PUT %s not implemented" % (self.__class__.__name__)
        )
    ...
    def _delete(self, controller):
        ...
        if self.id is UnsetParameter:
            raise KayakoRequestError(
                "Cannot delete a non-existent %s. The ID of the %s to delete has not been specified."
    ...
    def delete(self):
        ...
        raise KayakoMethodNotImplementedError(
            "DELETE %s not implemented" % (self.__class__.__name__)
        )
